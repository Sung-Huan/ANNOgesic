...
def get_feature(cds, cds_name):
    if "protein_id" in cds.attributes.keys():
        ...
        cds_name = cds.feature + ":" + str(cds.start) + "-" + \
    ...
    if detect:
        ...
        if source is True:
            attribute_string = ";".join(
                 ["=".join(items) for items in [("ID", "utr5_" + str(num_utr)),
                  ("Name", "5'UTR_" + name_utr), ("length", length),
                  ("TSS_type", tss.attributes["type"]),
                  ("associated_cds", cds_name),
                  ("associated_gene", locus_tag),
                  ("associated_tss", tss.attributes["Name"])]]) 
            out.write("%s\t%s\t5UTR\t%s\t%s\t.\t%s\t.\t%s\n") % \
                 (tss.seq_id, tss.source, str(start), str(end), tss.strand, attribute_string)
        else:
            attribute_string = ";".join(
                 ["=".join(items) for items in [("ID", "utr5_" + str(num_utr)),
                  ("Name", "5'UTR_" + name_utr), ("length", length),
                  ("associated_cds", cds_name),
                  ("associated_gene", locus_tag),
                  ("associated_tss","TSS_" + str(tss.start) + tss.strand)]])
            out.write("%s\t%s\t5UTR\t%s\t%s\t.\t%s\t.\t%s\n") % \
                     (tss.seq_id, tss.source, str(start), str(end), tss.strand, attribute_string)
        num_utr += 1
    return num_utr
...
def Detect_5UTR(TSS_file, GFF_file, TA_file, source, out_file):
    ...
    cdss = []
    tsss = []
    utrs_all = {"all": [], "pri": [], "sec": []}
    utrs_strain = {"all": {}, "pri": {}, "sec": {}}
    pre_seq_id = ""
    out = open(out_file, "w")
    ...
    for tss in tsss:
        ...
        if utr_datas["check"]:
            ...
            num_utr = get_5utr(tss, utr_datas["near_cds"], utr_strain, utr_all, tas, num_utr, 
                              utr_datas["cds_name"], utr_datas["locus_tag"], source, out)
    name = (GFF_file.split("/"))[-1].replace(".gff", "")
    plot(utr_all["all"], utr_all["pri"], utr_all["sec"], 
         name + "_all_5utr_length.png", source, "5utr")
    if len(utr_strain["all"]) > 1:
        for strain in utr_strain["all"].keys():
            plot(utr_strain["all"][strain], utr_strain["pri"][strain],
                 utr_strain["sec"][strain], strain + "_5utr_length.png",
                 source, "5utr")

def compare_term(ta, terms):
    for term in terms:
        if ta.strand == term.strand:
            if term.strand == "+":
                if (math.fabs(ta.end - term.start) <= args.fuzzy) or \
                   ((ta.end >= term.start) and (ta.end <= term.end)):
                    return term
            else:
                if (math.fabs(ta.start - term.end) <= args.fuzzy) or \
...
def get_3utr(ta, near_cds, utr_all, utr_strain, attributes, num_utr, out):
    if ta.strand == "+":
        ...
        utr_all.append(length)
        utr_strain[ta.seq_id].append(length)
    attributes.append("lenght=" + str(length))
    attributes.append("associated_tran=" + ta.attributes["ID"])
    attribute = ";".join(attributes)
    if (length <= 300) and (length > 0):
        name_utr='%0*d' % (5, num_utr)
        name = "Name=3'UTR_" + ta.attributes["ID"]
        ...
        for gene in genes:
            if ("Parent" in near_cds.attributes.keys()):
                if near_cds.attributes["Parent"] == gene.attributes["ID"]:
                    attributes.append("associated_gene=" + gene.attributes["locus_tag"])
        if "protein_id" in near_cds.attributes.keys():
            attributes.append("associated_cds=" + near_cds.attributes["protein_id"])
        elif "locus_tag" in near_cds.attributes.keys():
            attributes.append("associated_cds=" + near_cds.attributes["locus_tag"])
        else:
            attributes.append("associated_cds=" + "_".join([near_cds.feature,
...
def Detect_3UTR(TA_file, GFF_file, term_file, fuzzy, out_file):
    ...
    for ta in tas:
        if term_file is not False:
            term = compare_term(ta, terms)
        else:
            term = None
        attributes = []
        if term is not None:
            attributes.append("associated_term=" + term.attributes["ID"])
    ...
    name = (GFF_file.split("/"))[-1].replace(".gff", "")
    plot(utr_all, None, None, name + "_all_3utr_length.png", None, "3utr")
    if len(utr_strain) > 1:
        for strain in utr_strain.keys():
            plot(utr_strain[strain], None, None, strain + "_3utr_length.png", None, "3utr")
