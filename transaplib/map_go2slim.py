...
def _get_label(class_, classes, percent, num):
    if percent >= 3:
        ...
        if len(class_) > 30:
            ...
            name = class_
        classes.append(("%s %s (%s%s)") % (name, num, round(percent, 2), "%"))
...
def _plot(total_nums, strain, filename, total):
    ...
    for total_num in sort_total_nums:
        ...
        if class_ != "total":
            ...
            if count <= 5:
                ...
                _random_color(count, colors, color, color_elements)
            count += 1
    ...
    plt.savefig(strain + "_" + filename + ".png")
...
def _print_file(classes, total_nums):
    out_stat = open(args.stat, "w")
    printed = True
    for strain, datas in classes.items():
        if (strain == "All_strain") and len(classes) <= 2:
            printed = False
        if printed:
            _plot(total_nums[strain], strain, "three_roots", total_nums[strain]["total"])
            out_stat.write("%s:\n" % strain)
            for origin, types in datas.items():
                _plot(types, strain, origin, total_nums[strain][origin])
                out_stat.write("\t%s: %s(percentage in total: %s)\n" %  \
                              (origin, total_nums[strain][origin], \
                               float(total_nums[strain][origin]) / float(total_nums[strain]["total"])))
                for type_, num in types.items():
                    out_stat.write("\t\t%s: %s(percentage in %s: %s)\n" % \
...
def Map2Goslim(slim_file, term_file, go_table, stat):
    gos = {}
    term_obos = []
    slim_obos = []
    classes = {}
    ...
    for row in csv.reader(g_h, delimiter="\t"):
        if row[0] != "strain":
            if row[0] != pre_strain:
                gos[row[0]] = []
                _initiate_dict(classed, total_nums, row[0])
            go_terms = row[-1].split("; ")
            gos[row[0]] = gos[row[0]] + go_terms
            pre_strain = row[0]
    print("load obo file")
    _import_obo(args.term_file, term_obos)
    _import_obo(args.slim_file, slim_obos)
    print("start mapping")
    _compare_go_slim(gos, term_obos, slim_obos, classes, total_nums)
    _print_file(classes, total_nums)
