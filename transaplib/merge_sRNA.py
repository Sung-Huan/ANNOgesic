        ...
        if pre_srna.attributes["UTR_type"] != srna.attributes["UTR_type"]:
            if input_type == "pre":
                if "&" not in pre_srna.attributes["UTR_type"]:
                    pre_srna.attributes["UTR_type"] = srna.attributes["UTR_type"] + "&" + \
                                                  pre_srna.attributes["UTR_type"]
            else:
                if "&" not in pre_srna.attributes["UTR_type"]:
                    srna.attributes["UTR_type"] = srna.attributes["UTR_type"] + "&" + \
                                                  pre_srna.attributes["UTR_type"]
                else:
    ...
    if (pre_srna.attributes["with_TSS"] == "NA") and \
       (srna.attributes["with_TSS"] != "NA"):
        pre_srna.attributes["with_TSS"] = srna.attributes["with_TSS"]
         ...
         (srna.attributes["with_TSS"] != "NA"):
        pre_srna.attributes["with_TSS"] = pre_srna.attributes["with_TSS"] + "&" + srna.attributes["with_TSS"]
...
def compare_table(srna, tables, type_, wigs_f, wigs_r, template_texs, table_best, out):
    detect = False
    ...
    if detect is not True:
        ...
        srna_datas = srna_datas = Repliate_comparison(srna_covers, template_texs, strand,
        ...
        if srna_datas is not None:
            ...
            if table_best is False:
                ...
                for data in srna_datas["detail"]:
                    if first:
                        out.write("%s(avg=%s;high=%s;low=%s)" % (
                    ...
                    for cover in covers:
                        if (cover["strand"] == strand):
                            if (start <= cover["pos"]) and \
                               (end >= cover["pos"]):
                                cover["total"] = cover["total"] + cover["coverage"]
...
def get_tss_pro(type_, srna):
    if type_ == "utr":
        if (srna.attributes["with_TSS"] != "NA") and \
           (srna.attributes["with_cleavage"] != "NA"):
            tss_pro = srna.attributes["with_TSS"] + ";" + srna.attributes["with_cleavage"]
