...
#        self._set_static_files()
    ...
    def _set_folder_names(self):
        ...
        self.version_path = "%s/used_transap_version.txt" % (self.base_path)
    ...
    def _set_input_folder_names(self):
        ...
        self.ribos_folder = "%s/riboswitch_ID" % self.input_folder
    ...
    def _set_reference_input_folder_names(self):
        ...
        self.ref_annotation_folder = "%s/annotation" % (
            self.reference_base_folder)
        self.ref_fasta_folder = "%s/fasta" % (
            self.reference_base_folder)
    ...
    def _set_tsspredator_folder_names(self):
        self.tsspredator_base_folder = "%s/TSS" % self.output_folder
    ...
    def _set_processing_folder_names(self):
        self.processing_base_folder = "%s/processing_site" % self.output_folder
    ...
    def _set_transterm_folder_names(self):
        ...
        self.term_to_table_folder = "%s/tables" % (
            self.transterm_base_folder)
        self.transtermhp_folder = "%s/transtermhp" % (
            self.transterm_base_folder)
    ...
    def _set_srna_folder_names(self):
        ...
        self.srna_table_folder = "%s/tables" % (
            self.srna_base_folder)
        self.srna_sec_folder = "%s/sec_structure" % (
            self.srna_base_folder)
        self.srna_sec_plot_folder = "%s/sec_structure/sec_plot" % (
            self.srna_base_folder)
        self.srna_dot_plot_folder = "%s/sec_structure/dot_plot" % (
            self.srna_base_folder)
        self.srna_mountain_folder = "%s/mountain_plot" % (
            self.srna_base_folder)
        self.srna_blast_folder = "%s/blast_result_and_misc" % (
            self.srna_base_folder)
        self.srna_stat_folder = "%s/statistics" % (
            self.srna_base_folder)
        self.srna_gff_class_folder = "%s/for_class" % (
            self.srna_gff_folder)
        self.srna_gff_best_folder = "%s/best" % (
            self.srna_gff_folder)
        self.srna_gff_all_folder = "%s/all_candidates" % (
            self.srna_gff_folder)
        self.srna_table_class_folder = "%s/for_class" % (
            self.srna_table_folder)
        self.srna_table_best_folder = "%s/best" % (
            self.srna_table_folder)
        self.srna_table_all_folder = "%s/all_candidates" % (
            self.srna_table_folder)

    def _set_sorf_folder_names(self):
        self.sorf_base_folder = "%s/sORF" % self.output_folder
        ...
        self.sorf_stat_folder = "%s/statistics" % (
            self.sorf_base_folder)
        self.sorf_gff_best_folder = "%s/best" % (
            self.sorf_gff_folder)
        self.sorf_gff_all_folder = "%s/all_candidates" % (
            self.sorf_gff_folder)
        self.sorf_table_best_folder = "%s/best" % (
    ...
    def _set_circrna_folder_names(self):
        self.circrna_base_folder = "%s/circRNA" % self.output_folder
        self.circrna_align_folder = "%s/segemehl_align" % (
             self.circrna_base_folder)
        self.circrna_splice_folder = "%s/segemehl_splice" % (
             self.circrna_base_folder)
    ...
    def _set_starget_folder_names(self):
        ...
        self.starget_RNAplex_folder = "%s/RNAplex" % (
             self.starget_base_folder)
        self.starget_RNAup_folder = "%s/RNAup" % (
             self.starget_base_folder)
        self.starget_merge_folder = "%s/merge" % (
             self.starget_base_folder)
    ...
    def _set_snp_folder_names(self):
        ...
        self.snp_tar_stat_folder = "%s/statistics" % (
             self.tar_snp_folder)
        self.snp_ref_table_folder = "%s/SNP_table" % (
             self.ref_snp_folder)
        self.snp_tar_table_folder = "%s/SNP_table" % (
             self.tar_snp_folder)
    ...
    def _set_ppi_folder_names(self):
        self.ppi_base_folder = "%s/PPI" % self.output_folder
    ...
    def required_input_folders(self):
		...
		self.promoter_input_folder, self.database_folder,
                ...
                self.ribos_folder, self.manual_pro_folder]
