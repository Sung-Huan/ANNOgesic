...
class sRNA_detection(object):
    ...
    def __init__(self, out_folder, tsss, pros, sORF, fastas, trans):
        ...
        self.prefixs = {"merge": os.path.join(out_folder, "tmp_merge"),
                        ...
                        "merge_table": os.path.join(out_folder, "tmp_merge_table"),
                        "utr_table": os.path.join(out_folder, "tmp_utrsrna_table"),
                        "normal_table": os.path.join(out_folder, "tmp_normal_table"),
...
#                                          utr3_coverage, interCDS_coverage, out_folder)
                ...
                self.helper.sort_gff(merge_gff, "_".join([self.prefixs["basic"], prefix]))
        return prefixs
    ...
    def _get_seq_sec(self, fasta_path, out_folder, prefix, sec_path, dot_path, vienna_path):
        ...
        os.system(" ".join(["cat", seq_file, "|", os.path.join(vienna_path, "Progs", "RNAfold"),
                            "-p", ">", sec_file]))
        extract_energy(os.path.join(main_path, "_".join([self.prefixs["basic"], prefix])), 
                       sec_file, os.path.join(main_path, "_".join([self.prefixs["energy"], prefix])))
    ...
    def _blast(self, database, database_format, data_type, out_folder, blast_path,
               prefixs, fasta_path, program, database_type, e):
        if (database is None):
    ...
    def _class_srna(self, import_info, prefixs, gff_output, table_output, stat_path,
        ...
        if (len(import_info) != 1) or ('6' not in import_info):
            for prefix in prefixs:
                ...
                classify_srna(os.path.join(self.all_best["all_gff"], "_".join([prefix, "sRNA.gff"])), 
    ...
    def _get_best_result(self, prefixs, gff_output, table_output, energy, nr_hit_num,
        ...
        for prefix in prefixs:
            ...
            gen_best_srna(os.path.join(self.all_best["all_gff"], "_".join([prefix, "sRNA.gff"])), 
                          all_hit, energy, nr_hit_num, best_sORF, best_gff)
            gen_srna_table(os.path.join(self.all_best["best_gff"], "_".join([prefix, "sRNA.gff"])),
                           "_".join([self.prefixs["merge_table"], prefix]),
                           "_".join([self.tmps["nr"], prefix + ".csv"]),
                           "_".join([self.tmps["srna"], prefix + ".csv"]),
...
#                os.rename(tmp_srna, "_".join([self.prefixs["basic"], prefix]))
    ...
    def run_srna_detection(self, vienna_path, blast_path, ps2pdf14_path, out_folder, utr_srna, 
                           ...
                           nr_hits_num, sORF, all_hit, best_sORF):
        ...
        self._filter_srna(import_info, out_folder, prefixs, self.fasta_path, vienna_path, mountain,
                     ps2pdf14_path, nr_database, database_format, blast_path, srna_database,
                     self.stat_path, self.sorf_path, e_nr, e_srna)
#        for prefix in prefixs:
...
#            gen_srna_table(os.path.join(self.all_best["all_gff"], "_".join([prefix, "sRNA.gff"])),
#                           "_".join([self.prefixs["merge_table"], prefix]),
#                           "_".join([self.tmps["nr"], prefix + ".csv"]),
#                           "_".join([self.tmps["srna"], prefix + ".csv"]),
#                           max_len, min_len, out_table)
