...
class sRNATargetPrediction(object):
    ...
    def _run_rnaplex(self, target_seq_path, prefix, rnaplex_path,
    ...
    def _rna_plex(self, prefixs, rnaplex_path, vienna_path, win_size_s, span_s,
        ...
        for prefix in prefixs:
            ...
            self._run_rnaplfold(vienna_path, "target", win_size_t, span_t,
                                ...
                                prefix, rnaplfold_path)
            num_process = self._run_rnaplex(target_seq_path, prefix,
                               rnaplex_path, vienna_path, srna_seq_path,
                               inter_length, energy, duplex_dist, rnaplfold_path,
                               core_plex)
            rnaplex_file = os.path.join(rnaplex_path, prefix,
                           "_".join([prefix, "RNAplex.txt"]))
            if "_".join([prefix, "RNAplex.txt"]) in \
                   os.listdir(os.path.join(rnaplex_path, prefix)):
            ...
            for index in range(0, num_process):
                self.helper.merge_file(os.path.join(rnaplex_path, prefix,
                                       "_".join([prefix, "RNAplex",
                                           str(index) + ".txt"])),
                                       rnaplex_file)
            self.helper.remove_all_content(os.path.join(rnaplex_path, prefix),
                                           "_RNAplex_", "file")
            self.fixer.fix_rnaplex(rnaplex_file, self.tmps["tmp"])
            shutil.move(self.tmps["tmp"], rnaplex_file)

    def _run_rnaup(self, num_up, vienna_path, unstr_region_rnaup,
                   processes, out_folder, out_rnaup, out_log):
        for index in range(1, num_up + 1):
            out_tmp_up = open(os.path.join(out_folder,
                              "".join([self.tmps["rnaup"],
                                  str(index), ".txt"])), "w")
            out_err = open(os.path.join(out_folder,
                           "".join([self.tmps["log"], str(index), ".txt"])),
                           "w")
        ...
        if len(processes) != 0:
            time.sleep(5)
            self._wait_process(processes)
            os.system("rm " + os.path.join(out_folder, self.tmps["all_fa"]))
            self._merge_txt(num_up, out_rnaup, out_log, out_folder)
            os.system("rm " + os.path.join(out_folder, self.tmps["all_txt"]))

    def _merge_txt(self, num_up, out_rnaup, out_log, out_folder):
        for index in range(1, num_up + 1):
            self.helper.merge_file(os.path.join(out_folder,
                "".join([self.tmps["rnaup"], str(index), ".txt"])), out_rnaup)
            self.helper.merge_file(os.path.join(out_folder,
                "".join([self.tmps["log"], str(index), ".txt"])), out_log)
    ...
    def _rnaup(self, prefixs, rnaup_path, srna_seq_path, target_seq_path,
               core_up, vienna_path, unstr_region_rnaup, out_folder, con_up):
        for prefix in prefixs:
            srnas = []
            print("Running RNAup of {0}".format(prefix))
#            self.helper.check_make_folder(os.path.join(rnaup_path, prefix))
            if not os.path.exists(os.path.join(rnaup_path, prefix)):
                os.mkdir(os.path.join(rnaup_path, prefix))
            num_up = 0
            processes = []
            out_rnaup = os.path.join(rnaup_path, prefix,
                        "_".join([prefix + "_RNAup.txt"]))
            out_log = os.path.join(rnaup_path, prefix,
                      "_".join([prefix + "_RNAup.log"]))
            if "_".join([prefix, "RNAup.txt"]) in \
                    os.listdir(os.path.join(rnaup_path, prefix)):
                if not con_up:
                    os.remove(out_rnaup)
                    os.remove(out_log)
                else:
                    srnas = self._get_continue(out_rnaup)
            with open(os.path.join(srna_seq_path, "_".join([self.tmps["tmp"],
                      prefix, "sRNA.fa"])), "r") as s_f:
                for line in s_f:
                    line = line.strip()
                    if line.startswith(">"):
                        if line[1:] in srnas:
                        ...
                        num_up += 1
                        out_up = open(os.path.join(out_folder,
                                      "".join([self.tmps["tmp"],
                                          str(num_up), ".fa"])), "w")
                        out_up.write(line + "\n")
                    else:
                        if start:
                            out_up.write(line + "\n")
                            out_up.close()
                            self.helper.merge_file(os.path.join(target_seq_path,
                                "_".join([prefix, "target.fa"])), os.path.join(out_folder,
                                "".join([self.tmps["tmp"], str(num_up), ".fa"])))
                            if num_up == core_up:
                                self._run_rnaup(num_up, vienna_path,
                                     unstr_region_rnaup, processes, out_folder,
                                     out_rnaup, out_log)
                                processes = []
                                num_up = 0
            self._run_rnaup(num_up, vienna_path, unstr_region_rnaup,
                            processes, out_folder, out_rnaup, out_log)
    ...
    def _merge_rnaplex_rnaup(self, prefixs, merge_path, program, rnaplex_path,
        ...
        for prefix in prefixs: # merge and rank the sRNA target output
            ...
            if (program == "both") or (program == "RNAplex"):
                rnaplex_file = os.path.join(rnaplex_path, prefix,
                                            "_".join([prefix, "RNAplex.txt"]))
                out_rnaplex = os.path.join(rnaplex_path, prefix,
                                           "_".join([prefix, "RNAplex_rank.csv"]))
            if (program == "both") or (program == "RNAup"):
                rnaup_file = os.path.join(rnaup_path, prefix,
                                          "_".join([prefix, "RNAup.txt"]))
                out_rnaup = os.path.join(rnaup_path, prefix,
                                         "_".join([prefix, "RNAup_rank.csv"]))
            merge_srna_target(rnaplex_file, rnaup_file, top,
                              out_rnaplex, out_rnaup,
                              os.path.join(merge_path, prefix,
    ...
    def run_srna_target_prediction(self, vienna_path, gffs, fastas, srnas,
        ...
        if (program == "both") or \
           (program == "RNAplex"):
            self._rna_plex(prefixs, self.rnaplex_path, vienna_path, win_size_s,
                           span_s, unstr_region_rnaplex_s, self.srna_seq_path,
                           win_size_t, span_t, unstr_region_rnaplex_t,
                           self.target_seq_path, inter_length, energy,
                           duplex_dist, core_plex)
        self.helper.remove_all_content(self.target_seq_path, "_target_", "file")
        if (program == "both") or \
           (program == "RNAup"):
            self._rnaup(prefixs, self.rnaup_path, self.srna_seq_path,
                        self.target_seq_path, core_up, vienna_path,
                        unstr_region_rnaup, out_folder, continue_rnaup)
        self._merge_rnaplex_rnaup(prefixs, self.merge_path, program,
             self.rnaplex_path, self.rnaup_path, self.srna_path,
             self.gff_path, top)
        if (program == "RNAplex") or \
           (program == "both"):
            for strain in os.listdir(os.path.join(out_folder, "RNAplex")):
                shutil.rmtree(os.path.join(
                              out_folder, "RNAplex", strain, "RNAplfold"))
        self.helper.remove_all_content(out_folder, self.tmps["tmp"], "dir")
        self.helper.remove_all_content(out_folder, self.tmps["tmp"], "file")
