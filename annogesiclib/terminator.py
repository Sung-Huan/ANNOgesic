...
class Terminator(object):
    ...
    def __init__(self, gffs, fastas, trans, out_folder, sRNAs):
        ...
        self.tmps = {"transterm": os.path.join(os.getcwd(), "tmp_transterm"),
                     ...
                     "gff": "tmp.gff", "folder": os.path.join(os.getcwd(), "tmp")}
        self.suffixs = {"gff": "term.gff", "csv": "term.csv", "allgff": "term_all.gff"}
        if sRNAs:
    ...
    def _convert_to_gff(self, prefixs, hp_folder, gffs):
        ...
        self.multiparser._combine_gff(gffs, self.tmps["transterm"], None, self.tmps["hp"])
    ...
    def _move_file(self, term_outfolder, csv_outfolder):
        for gff in os.listdir(term_outfolder):
            if gff.endswith("_term.gff"):
                ...
                new_gff = os.path.join(self.terms["all"], "_".join([prefix, self.suffixs["allgff"]]))
                ...
                for entry in self.gff_parser.entries(open(new_gff)):
                    if entry.seq_id != pre_strain:
                        self.helper.merge_file(os.path.join(self.tmps["term_table"], 
    ...
    def _compute_intersection_forward_reverse(self, RNAfold_path, prefixs, tran_path, 
                        ...
                        table_best, gffs, out_folder, fuzzy_up_ta, fuzzy_down_ta, 
                        fuzzy_up_cds, fuzzy_down_cds):
        for prefix in prefixs:
            tmp_seq = os.path.join(out_folder, "_".join(["inter_seq", prefix]))
            tmp_sec = os.path.join(out_folder, "_".join(["inter_sec", prefix]))
            tran_file = os.path.join(tran_path, "_".join([prefix, "transcript.gff"]))
            ...
            poly_t(tmp_seq, tmp_sec, gff_file, tran_file, fuzzy_up_ta, fuzzy_down_ta, 
                   fuzzy_up_cds, fuzzy_down_cds, tmp_cand)
            ...
            detect_coverage(tmp_cand, 
                            ...
                            os.path.join(self.tmps["hp_path"], "_".join([prefix, self.tmps["hp_gff"]])), 
                            merge_wigs, libs, tex_notex, replicates, 
                            os.path.join(term_outfolder, "_".join([prefix, self.suffixs["gff"]])), 
                            os.path.join(self.tmps["term_table"], "_".join([prefix, "term_raw.csv"])), 
                            table_best, decrease)
    ...
    def _compute_stat(self, term_outfolder, csv_outfolder, stat, out_folder):
        ...
        if stat:
            ...
            for prefix in new_prefixs:
                stat_term(os.path.join(self.terms["all"], "_".join([prefix, self.suffixs["gff"]])), 
                          os.path.join(self.csvs["all"], "_".join([prefix, self.suffixs["csv"]])), 
                          os.path.join(stat_path, "_".join(["stat", prefix + ".csv"])), 
                          os.path.join(self.terms["detect"], "_".join([prefix, "term"])), 
                          os.path.join(self.terms["express"], "_".join([prefix, "term"])))
                os.rename(os.path.join(self.terms["detect"], "_".join([prefix, self.suffixs["csv"]])), 
                          os.path.join(self.csvs["detect"], "_".join([prefix, self.suffixs["csv"]])))
                os.rename(os.path.join(self.terms["express"], "_".join([prefix, self.suffixs["csv"]])),
                          os.path.join(self.csvs["express"], "_".join([prefix, self.suffixs["csv"]])))
    ...
    def run_terminator(self, TransTermHP_path, RNAfold_path, out_folder, fastas, gffs, 
                       ...
                       fuzzy, fuzzy_up_ta, fuzzy_down_ta, fuzzy_up_cds, fuzzy_down_cds, 
                       hp_folder, tlibs, flibs, tex_notex, replicates_tex, replicates_frag, table_best):
        ...
        self.multiparser._parser_gff(self.tmps["transterm"], self.tmps["hp"])
        ...
        self._compute_intersection_forward_reverse(RNAfold_path, prefixs, self.tran_path, merge_path, 
                    ...
                    replicates, decrease, self.outfolder["term"], self.outfolder["csv"], table_best, gffs, 
                    out_folder, fuzzy_up_ta, fuzzy_down_ta, fuzzy_up_cds, fuzzy_down_cds)
        self._compute_stat(self.outfolder["term"], self.outfolder["csv"], stat, out_folder)
        self._remove_tmp_file(gffs, fastas, sRNAs, tex_wigs, frag_wigs, self.outfolder["term"], out_folder, merge_wigs)
