...
import annogesiclib.parser_wig as pw

def load_wigs(strand, out, lib_t, lib_n, lib_f):
    if lib_t and lib_n:
...
def set_data_range(out, gff, wigs, strand):
    max_range = 0
    for filename, strains in wigs.items():
...
def print_batch(out, strand, lib_t, lib_n, lib_f, fasta, main_gff, presentation, 
    ...
    for filename in side_gffs:
        ...
        gff = filename.split("/")
        out.write("{0} {1}\n".format(presentation, gff[-1]))
    load_wigs(strand, out, lib_t, lib_n, lib_f)
    out.write("maxPanelHeight {0}\n".format(height))
    if strand == "+":
        out.write("snapshotDirectory {0}\n".format(
                  os.path.join(os.getcwd(), screenshot_folder, strain, "forward"))) 
    else:
        out.write("snapshotDirectory {0}\n".format(
                  os.path.join(os.getcwd(), screenshot_folder, strain, "reverse")))   

def convert_sci_number(number):
    order = 1
    rev_start = ""
    for digit in reverse(number):
        if order == 4:
            order = 1
            rev_start + ","
        rev_start = rev_start + digit
        order += 1
    return rev_start.reverse()

def import_wig(lib, wigs, strand):
    wig_parser = pw.parser_wig()
    for wig in lib:
        wigs[wig] = {}
        strain = ""
        for entry in wig_parser.parser(wig, strand):
...
def gen_batch(lib_t, lib_n, lib_f, strand, gffs, out):
    ...
    for gff in gffs:
        out.write("goto {0}:{1}-{2}\n".format(gff.seq_id, gff.start - 200, gff.end + 200))
        set_data_range(out, gff, wigs, strand)
        out.write("snapshot {0}:{1}-{2}.png\n".format(gff.seq_id, gff.start, gff.end))

def gen_screenshot(main_gff, forward_file, reverse_file, screenshot_folder, height,
                   lib_ft, lib_fn, lib_rt, lib_rn, lib_ff, lib_rf, fasta, side_gffs, 
                   presentation, strain):
    gffs_f = []
    gffs_r = []
    ...
    print_batch(out_f, "+", lib_ft, lib_fn, lib_ff, fasta, main_gff, presentation, 
                side_gffs, height, screenshot_folder, strain)
    out_r = open(reverse_file, "w")
    print_batch(out_r, "-", lib_rt, lib_rn, lib_rf, fasta, main_gff, presentation, 
                side_gffs, height, screenshot_folder, strain)
