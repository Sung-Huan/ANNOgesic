...
class RATT(object):
    ...
    def __init__(self, ref_embls, output_path, tar_fastas, ref_fastas, gff_outfolder):
    ...
    def _convert_to_pttrnt(self, gffs, fastas):
        for gff in os.listdir(gffs):
            if gff.endswith(".gff"):
                ...
                rnt = gff[:-3] + "rnt"
                ptt = gff[:-3] + "ptt"
                fasta = self.helper.get_correct_file(self.tmp_files["tar"], ".fa", prefix, None)
                if fasta:
    ...
    def _remove_files(self, gff_outfolder, tar_fastas, out_gbk, ref_fastas):
        ...
        self.helper.move_all_content(self.tmp_files["out_gff"], gff_outfolder, None)
    ...
    def _run_ratt(self, output_path, tar_fastas, ratt_path,
                  element, transfer_type, ref_fastas, pairs):
        print("Running RATT...")
        for pair in pairs:
            ref = pair.split(":")[0]
            tar = pair.split(":")[1]
            ...
            call([os.path.join(ratt_path, "start.ratt.sh"),
                  ...
                  element, transfer_type, os.path.join(self.tmp_files["ref"], ref + ".fa")],
    ...
    def annotation_transfer(self, ratt_path, pagit_folder, element, transfer_type, ref_embls, 
                            tar_fastas, ref_fastas, output_path, convert, gff_outfolder, pairs):
                ...
                for gff in os.listdir(gff_outfolder):
                    for file_ in files:
                        if (".gff" in gff) and (file_ == gff[:-4]):
                            self.helper.merge_file(os.path.join(gff_outfolder, gff), self.tmp_files["gff"])
                        if (".ptt" in gff) and (file_ == gff[:-4]):
                            self.helper.merge_file(os.path.join(gff_outfolder, gff), self.tmp_files["ptt"])
                        if (".rnt" in gff) and (file_ == gff[:-4]):
                            self.helper.merge_file(os.path.join(gff_outfolder, gff), self.tmp_files["rnt"])
                os.rename(self.tmp_files["gff"], os.path.join(self.tmp_files["out_gff"], prefix + ".gff"))
                os.rename(self.tmp_files["ptt"], os.path.join(self.tmp_files["out_gff"], prefix + ".ptt"))
                os.rename(self.tmp_files["rnt"], os.path.join(self.tmp_files["out_gff"], prefix + ".rnt"))
