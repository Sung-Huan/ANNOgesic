                ...
                for strain in strains:
                    if strain + ".fa" in os.listdir(self.folders["tmp_tar"]):
                        with open(os.path.join(
                                  self.folders["tmp_tar"], strain + ".fa")) as f_h:
                            for line in f_h:
                                out.write(line)
                    else:
                        print("Error:There is no fasta information of {0}.fa".format(strain))
                out.close()
        else:
            self.helper.move_all_content(self.folders["tmp_tar"], ".fa ", tar_folder)
        shutil.rmtree(self.folders["tmp_tar"])
