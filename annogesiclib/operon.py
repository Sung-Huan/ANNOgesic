...
#            self._check_gff(terms, "term")
    ...
    def _detect_operon(self, prefixs, gffs, tss_fuzzy, term_fuzzy, length):
        for prefix in prefixs:
            out_table = os.path.join(self.table_path, "_".join(["operon", prefix + ".csv"]))
            print("Detection operons of {0}".format(prefix))
            tss = self.helper.get_correct_file(self.tss_path, "_TSS.gff", prefix, None)
            tran = self.helper.get_correct_file(self.tran_path, "_transcript.gff", prefix, None)
            gff = self.helper.get_correct_file(gffs, ".gff", prefix, None)
            if self.term_path is None:
                term = False
            else:
                term = self.helper.get_correct_file(self.term_path, "_term.gff", prefix, None)
...
#            self._check_gff(terms, "term")
    ...
    def _combine_gff(self, prefixs, output_folder, gffs, tss_fuzzy, term_fuzzy):
        for prefix in prefixs:
            ...
            tran = self.helper.get_correct_file(self.tran_path, "_transcript.gff", prefix, None)
            gff = self.helper.get_correct_file(gffs, ".gff", prefix, None)
            utr5 = self.helper.get_correct_file(self.utr5_path, "_5UTR.gff", prefix, None)
            utr3 = self.helper.get_correct_file(self.utr3_path, "_3UTR.gff", prefix, None)
            if self.term_path is None:
                term = None
            else:
                term = self.helper.get_correct_file(self.term_path, "_term.gff", prefix, None)
    ...
    def run_operon(self, tsss, gffs, trans, utr5s, utr3s, terms, tss_fuzzy, term_fuzzy, 
        ...
        self._check_and_parser_gff(tsss, gffs, trans, utr5s, utr3s, 
                                   output_folder, terms)
        prefixs = []
        for gff in os.listdir(gffs):
            if gff.endswith(".gff"):
