...
def import_utr(source, tss, utr_strain, utr_all, start, end, tas, length, base_5utr):
    if source:
                    ...
                    for cds in cdss:
                        if (cds.seq_id == tss.seq_id) and \
                           (cds.strand == tss.strand):
                            if locus_tag == (cds.feature + ":" + str(cds.start) + \
                                             "-" + str(cds.end) + "_" + cds.strand):
                                near_cds = cds
                                cds_name = get_feature(cds)
                                check_utr = True
...
def compare_ta_cds_gene(cds, ta, tsss, genes, utr_start, utr_end, pre_pos, source):
    detect = False
...
def compare_ta(tas, tsss, genes, cdss, utr_strain, utr_all, out, source):
    ...
    for ta in tas:
                ...
                if (ta.seq_id == cds.seq_id) and \
                   (ta.strand == cds.strand):
                    if (cds.start >= ta.start) and \
                       (cds.start <= ta.end):
                        datas = compare_ta_cds_gene(cds, ta, tsss, genes, ta.start, 
                                                    cds.start, pre_pos, source)
                    pre_pos = cds.end
        else:
                ...
                if (ta.seq_id == cds.seq_id) and \
                   (ta.strand == cds.strand):
                    if (cds.end <= ta.end) and \
                       (cds.end >= ta.start):
                        datas = compare_ta_cds_gene(cds, ta, tsss, genes, cds.end, 
                                                    ta.end, pre_pos, source)
                    pre_pos = cds.start
        ...
        if detect:
            ...
            out.write("{0}\t{1}\t5UTR\t{2}\t{3}\t.\t{4}\t.\t{5}\n".format(
                  ta.seq_id, ta.source, start, end, ta.strand, attribute_string))
            num_utr += 1
...
def detect_5utr(TSS_file, GFF_file, TA_file, source, base_5utr, out_file):
    ...
    read_file(TSS_file, tsss, GFF_file, cdss, genes, TA_file, tas, None, None)
        ...
        for tss in tsss:
            ...
            if utr_datas["check"]:
                ...
                num_utr = get_5utr(tss, utr_datas["near_cds"], utr_strain, utr_all, tas, num_utr, 
                                   utr_datas["cds_name"], utr_datas["locus"], source, out, base_5utr)
...
def detect_3utr(TA_file, GFF_file, term_file, fuzzy, out_file):
    ...
    out = open(out_file, "w")
    out.write("##gff-version 3\n")
    read_file(None, None, GFF_file, cdss, genes, TA_file, tas, term_file, terms)
    if term_file is not None:
        terms = sorted(terms, key=lambda k: (k.seq_id, k.start))
    genes = sorted(genes, key=lambda k: (k.seq_id, k.start))
    cdss = sorted(cdss, key=lambda k: (k.seq_id, k.start))
