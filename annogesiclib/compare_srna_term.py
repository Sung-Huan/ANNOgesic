...
def print_table(srna_table, out_t, srnas):
    ...
    for row in csv.reader(fh, delimiter='\t'):
        if first:
            ...
            for srna in srnas:
                if (row[1] == srna.seq_id) and (
                    int(row[3]) == srna.start) and (
                    int(row[4]) == srna.end) and (
                    row[5] == srna.strand):
                    if "term" in srna.attributes.keys():
                        row[-1] = srna.attributes["term"]
                    out_t.write("\t".join(row) + "\n")
            ...
            if (srna.seq_id == term.seq_id) and (
                ...
                if (srna.strand == "+"):
                    if (((srna.end - term.end) <= fuzzy_b) and (
                        ...
                        srna.attributes["term"] = term_string
                        out_g.write(srna.info + ";with_term=" + term_string + "\n")
                        detect = True
                        break
                else:
