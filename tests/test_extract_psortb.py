...
class TestExtractPsortb(unittest.TestCase):
    ...
    def test_get_results_unique(self):
        ...
        psortbs = []
        out_p = StringIO()
        local_name, local_score = ep.get_results(
            line, scores, psortbs, out_p, seq_name, 0.5)
        self.assertListEqual(local_name, [])
        self.assertListEqual(local_score, [])
        self.assertEqual(
            set(out_p.getvalue().split("\n")[:-1]),
            set(["Staphylococcus_aureus_HG003\tYP_498609.1\t+\t517\t1878\tCytoplasmic\t9.97"]))
        out_p.close()       
    ...
    def test_get_results_multi(self):
        ...
        psortbs = []
        out_p = StringIO()
        local_name, local_score = ep.get_results(line, scores, psortbs,
                                                 out_p, seq_name, 0.5)
        self.assertEqual(set(local_name), set([
    ...
    def test_import_psortb(self):
        seq_name = "Staphylococcus_aureus_HG003__YP_500332.1_-_1732031_1733725"
        psortbs = []
        results = ['Cytoplasmic', '(This', 'protein', 'may', 'have',
                   'multiple', 'localization', 'sites.)', '10.00']
        local_name = ['Cytoplasmic', 'CytoplasmicMembrane']
        local_score = ['10.0', '10.0']
        ep.import_psortb(seq_name, psortbs, local_name, local_score,
                         "multi", results)
        self.assertDictEqual(psortbs[0], {
            'score': '10.0/10.0', 'local': 'Cytoplasmic/CytoplasmicMembrane',
            'seq_id': 'Staphylococcus_aureus_HG003',
            'protein_id': 'YP_500332.1',
            'end': 1733725, 'strand': '-', 'start': 1732031})

    def test_print_gff(self):
        out_m = StringIO()
        psortbs = [{'score': '10.0/10.0',
                    'local': 'Cytoplasmic/CytoplasmicMembrane',
        ...
        for index in range(len(attributes)):
            self.assertEqual(set(attributes[index]),
                             set(ref_attributes[index].split(";")))
        out_m.close()
...
Staphylococcus_aureus_HG003	YP_500332.1	-	1732031	1733725	Cytoplasmic/CytoplasmicMembrane	10.0/10.0"""
    ...
    gff_dict = [{"seq_id": "aaa", "source": "Refseq", "feature": "gene",
                 "start": 100, "end": 140, "phase": ".", "strand": "+",
                 "score": "."},
                ...
                {"seq_id": "aaa", "source": "Refseq", "feature": "CDS",
                 ...
                 "score": "."}]
    attributes_gff = [{"ID": "gene0", "Name": "gene_0",
                       "locus_tag": "AAA_00001"},
                      {"ID": "cds0", "Name": "CDS_0",
                       "locus_tag": "AAA_00001",
                       "protein_id": "YP_500332.1"},
