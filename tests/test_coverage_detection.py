...
class Mock_func(object):
    ...
    def mock_check_tex(self, template_texs, covers, target_datas, cutoff,
                       tex_notex, poss, type_, notex, median,
                       coverages, utr_type):
...
class TestCoverageDetection(unittest.TestCase):
    ...
    def test_check_tex(self):
        ...
        poss = {"high": 30, "low": 10}
        median = {"track1_tex": {"median": 100, "mean": 200},
                  "track1_notex": {"median": 30, "mean": 80},
                  "track2_tex": {"median": 150, "mean": 200},
                  "track2_notex": {"median": 10, "mean": 20},
                  "frag": {"median": 80, "mean": 100}}
        target_datas = []
        texs = {"track1_tex@AND@track1_notex": 0,
                "track2_tex@AND@track2_notex": 0}
        detect_num_lib = cover_detect.check_tex(
            template_texs, covers, target_datas, 20, None, poss, median,
        ...
        detect_num_lib = cover_detect.check_tex(
            template_texs, covers, target_datas, 20, "sRNA_utr_derived",
            poss, median, coverages, "5utr", 200, 2)
        self.assertEqual(detect_num_lib, 2)
        self.assertDictEqual(poss, {
            'start': 100, 'high': 30, 'end': 202, 'low': 10})
...
class Example(object):
    cover_datas = [
        {"avg": 300, "type": "tex", "track": "track1_tex",
