...
class TestGenTableTran(unittest.TestCase):
    ...
    def test_detect_coverage(self):
        ...
        self.assertDictEqual(infos, {'track_1': {
            'high': 100, 'low': 2, 'avg': 33.529411764705884}})
    ...
    def test_print_coverage(self):
        ...
        self.assertListEqual(out_gff.getvalue().split("\t")[:-1],
                             ["aaa", "ANNOgesic", "Transcript",
                              "4", "20", ".", "+", "."])
        self.assertEqual(
            set(out_gff.getvalue().split("\t")[-1].strip().split(";")),
            set(["Name=Transcript_0", "detect_lib=fragmented&TEX+/-",
                 "best_avg_coverage=33.529411764705884", "ID=tran0"]))
class Example(object):
    wigs = {"aaa": {"frag_1": {"track_1|+|frag": [
        100, 30, 23, 21, 21, 2, 100, 30, 23, 21, 21, 2, 
        100, 30, 23, 21, 21, 2, 100, 30, 23, 21, 21, 2]}}}
    ta_dict = [{"seq_id": "aaa", "source": "ANNOgesic",
                "feature": "Transcript", "start": 4,
