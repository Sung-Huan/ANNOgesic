...
class TestStatTaComparison(unittest.TestCase):
    ...
    def test_detect_tas_region(self):
        ...
        self.assertDictEqual(stat, {'TSS_with_tran': 1, 'TSS_no_tran': 0,
                                    'no_TSS': 0, 'with_TSS': 1})

    def test_del_attributes(self):
        string = stc.del_attributes(self.example.tsss[0],
                                    ["utr_length", "libs"])
        self.assertDictEqual(string, {
            'type': 'Primary', 'associated_gene': 'SAOUHSC_00002',
            'Name': 'TSS:2131_f', 'ID': 'tss3'})

    def test_assign_tss(self):
        tsss = copy.deepcopy(self.example.tsss)
        trans = copy.deepcopy(self.example.tas)
        stc.assign_tss(tsss[0], trans[0])
        self.assertDictEqual(tsss[0].attributes, {
            'utr_length': 'Primary_25', 'ID': 'tss3', 'Name': 'TSS:2131_f',
            'libs': ("TSB_OD_0.2,TSB_OD_0.5,TSB_t0,"
                     "pMEM_OD_0.2,pMEM_OD_0.5,pMEM_t2"),
            'associated_gene': 'SAOUHSC_00002',
             'Parent': 'tran0', 'type': 'Primary'})
        self.assertDictEqual(trans[0].attributes, {
            'type': 'cover_CDS,cover_CDS',
            'Name': 'Transcript_00000',
            'associated_cds': 'YP_498609.1,YP_498610.1',
            'associated_tss': ("TSS:313_+,TSS:1641_+,TSS:2128_+,"
                               "TSS:2131_+,TSS:2131_f"),
            'ID': 'tran0'})

    def test_print_tas_stat(self):
        out = StringIO()
        stat = {'TSS_with_tran': 1, 'TSS_no_tran': 0,
                'no_TSS': 0, 'with_TSS': 1}
    ...
    def test_compare_tran_tss(self):
        ...
        self.assertDictEqual(stat, {'with_TSS': 2, 'no_TSS': 0,
                                    'TSS_no_tran': 0, 'TSS_with_tran': 1})
    ...
    def test_read_tag_file(self):
        ...
        gen_file(ta_file, self.example.ta)
        gffs, tas, stats, region = stc.read_tag_file(
            gff_file, ta_file, "gene")
        self.assertEqual(gffs[0].start, 517)
        self.assertEqual(tas[0].start, 313)
        self.assertEqual(stats, {'All': {'gene': 1, 'other': 0, 'bsbe': 0,
                                         'asbe': 0, 'asae': 0, 'bsae': 0},
                                 'aaa': {'gene': 1, 'other': 0, 'bsbe': 0,
                                         'asbe': 0, 'asae': 0, 'bsae': 0}})

    def test_detect_tag_region(self):
        stats = {'All': {'gene': 1, 'other': 0, 'bsbe': 0,
                         'asbe': 0, 'asae': 0, 'bsae': 0},
                 'aaa': {'gene': 1, 'other': 0, 'bsbe': 0,
                         'asbe': 0, 'asae': 0, 'bsae': 0}}
        out_t = StringIO()
        ...
        self.assertDictEqual(stats, {'All': {'asbe': 1, 'asae': 0, 'other': 0,
                                             'bsbe': 0, 'gene': 1, 'bsae': 0},
                                     'aaa': {'asbe': 1, 'asae': 0, 'other': 0,
                                             'bsbe': 0, 'gene': 1, 'bsae': 0}})

    def test_compare_ta_gff(self):
        tran_type = []
        check = [0, 0, 0, 0, 0]
        stats = {'All': {'asbe': 1, 'asae': 0, 'other': 0,
                         'bsbe': 0, 'gene': 1, 'bsae': 0},
                 'aaa': {'asbe': 1, 'asae': 0, 'other': 0,
                         'bsbe': 0, 'gene': 1, 'bsae': 0}}
        gffs = copy.deepcopy(self.example.gffs)
        trans = copy.deepcopy(self.example.tas)
        stc.compare_ta_gff(gffs, trans[0], check, tran_type,
                           False, stats, "gene")
        self.assertListEqual(check, [0, 0, 0, 1, 0])
        self.assertDictEqual(stats, {
            'aaa': {'asae': 0, 'bsbe': 0, 'asbe': 2,
                    'bsae': 0, 'other': 0, 'gene': 1},
            'All': {'asae': 0, 'bsbe': 0, 'asbe': 2,
                    'bsae': 0, 'other': 0, 'gene': 1}})
    ...
    def test_assign_parent(self):
        ...
        self.assertDictEqual(gffs[0].attributes, {
            'Parent': 'tran0',
...
aaa	Refseq	CDS	517	1878	.	+	.	protein_id=YP_498609.1;gene=dnaA;ID=cds0;Parent=gene0;locus_tag=SAOUHSC_00001;Name=YP_498609.1"""
    ...
    attributes_tss = [{"ID": "tss3", "Name": "TSS:2131_f",
    ...
    ta_dict = [{"seq_id": "aaa", "source": "fragmented_and_normal",
                "feature": "transcript",
                "start": 313, "end": 3344, "phase": ".",
                "strand": "+", "score": "."}]
    attributes_ta = [{"ID": "tran0", "Name": "Transcript_00000",
                      "associated_tss": ("TSS:313_+,TSS:1641_+,"
                                         "TSS:2128_+,TSS:2131_+"),
                      "type": "cover_CDS,cover_CDS",
                      "associated_cds": "YP_498609.1,YP_498610.1"}]
    tas = []
    ...
    gff_dict = [{"seq_id": "aaa", "source": "RefSeq", "feature": "gene",
                 "start": 517, "end": 1878, "phase": ".",
                 "strand": "+", "score": "."}]
    attributes_gff = [{"ID": "gene0", "Name": "YP_498609.1",
